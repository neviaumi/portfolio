jQuery(function ($) {
      ...
      .on('mouseover', mshotEnabledLinkSelector, function () {
        ...
        var mShot = $(
          '<div class="akismet-mshot mshot-container"><div class="mshot-arrow"></div><img src="' +
            akismet_mshot_url(linkUrl, mshotTries) +
        ...
        mShot.offset({
          ...
          ), // Keep it on the screen if the link is near the edge of the window.
  ...
  function retryMshotUntilLoaded() {
    ...
    if (imageWidth == 0) {
      ...
      if (!$('.akismet-mshot').data('pending-request')) {
        ...
        $('.akismet-mshot .mshot-image').attr(
          'src',
          akismet_mshot_url($('.akismet-mshot').data('url'), mshotTries),
        );
      }
  ...
  function akismet_check_for_spam(offset, limit) {
    ...
    var percentage_complete =
      Math.round(
        (recheck_count / check_for_spam_buttons.data('pending-comment-count')) *
          1000,
      ) / 10;

    // Update the progress counter on the "Check for Spam" button.
    $('.checkforspam').text(
      check_for_spam_buttons
        .data('progress-label')
  ...
  function akismet_enable_comment_author_url_removal() {
    $('#the-comment-list')
      ...
      .each(function () {
        ...
        var currentHost =
          currentHostParts[0] + '//' + currentHostParts[2] + '/';
  ...
  function akismet_mshot_url(linkUrl, retry) {
    var mshotUrl =
      '//s0.wp.com/mshots/v1/' + encodeURIComponent(linkUrl) + '?w=900';
